      ...
      <nav className="flex justify-between max-sm:sticky z-30 max-sm:bg-white/5 max-sm:rounded-full max-sm:py-3 max-sm:px-4 items-center">
        ...
        <ul className="lg:flex hidden flex-1 justify-center items-center gap-6">
          {navigation.map((nav, index) => (
            <Link
              className="gray-gradient-text uppercase"
              href={nav.href}
              key={index}
            >
              {nav.name}
        ...
        <div className="flex items-center gap-2">
          <CButton
            variant="borderless"
            className="uppercase max-sm:hidden flex max-sm:text-[14px] gray-gradient-text"
            icon={<ArrowUpRight color="gray" className="max-sm:text-[14px]" />}
          >
            Start a project
          </CButton>
          <Sheet>
            ...
            <SheetContent className="pt-20 flex lg:hidden bg-black border border-gray-500 flex-col overflow-y-auto items-start">
              ...
              {navigation.map((item, index) => (
                <SheetClose
                  className="w-full justify-start flex text-start"
                  key={index}
                >
                  <Link
                    className="text-[18px] capitalize py-4 border-b-[1px] w-full"
                    href={item.href}
                  >
                    {item.name}
                  </Link>
...
//     <Wrapper className="lg:py-4 py-4 max-sm:px-2">
//       <nav className="flex justify-between  max-sm:sticky z-30 max-sm:bg-white/5 max-sm:rounded-full max-sm:py-3 max-sm:px-4 items-center">
//         <span className="text-[20px] gradient-text font-semibold">
//           LucentLabs
//         </span>
//         <ul className="lg:flex hidden flex-1 justify-center items-center gap-6">
//           {navigation.map((nav, index) => (
